        ...
        try {
            for (String albumId : albumIds) {
                ...
                for (int i = 0; i < items.size(); i++) {
                    ...
                    JsonObject item = items.get(i).getAsJsonObject();
                    String artistName = items.get(0).getAsJsonObject().get("artists").getAsJsonArray().get(0).getAsJsonObject().get("names").getAsString();
                    String albumName = root.get("name").getAsString();
                    String coverURL = root.get("images").getAsJsonArray().get(0).getAsJsonObject().get("url").getAsString();
                    ArrayList<String> trackTitles = new ArrayList<>();
                    ArrayList<Integer> trackLengths = new ArrayList<>();

                    trackTitles.add(item.get("name").getAsString());
                    trackLengths.add(item.get("duration_ms").getAsInt()/1000);
